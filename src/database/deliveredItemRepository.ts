...
export async function createDeliveredItem(item: DeliveredItem): Promise<void> {
  await pool.query(
    `INSERT INTO delivered_items (id, title, source_url, type, author)
     VALUES ($1, $2, $3, $4, $5)
     ON CONFLICT (id) DO NOTHING`,
    [item.id, item.title, item.sourceUrl, item.type, item.author ?? null],
  );
}
...
export async function getDeliveredItem(id: string): Promise<DeliveredItem | null> {
  const { rows } = await pool.query<{
    ...
    type: DownloadType;
    author: string | null;
  }>('SELECT id, title, source_url, type, author FROM delivered_items WHERE id = $1', [id]);
  const row = rows[0];
  if (!row) return null;
  return { id: row.id, title: row.title, sourceUrl: row.source_url, type: row.type, author: row.author ?? undefined };
}
